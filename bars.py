...
def get_closest_bar(json_data, me_x_coordinate, me_y_coordinate):
    all_bars = json_data['features']
    bar = min(all_bars,
              key=lambda x: math.sqrt(((float(me_x_coordinate) -
                                        x['geometry']['coordinates'][0])**2) +
                                      ((float(me_y_coordinate) -
...
if __name__ == '__main__':
    ...
    closest_bar = get_closest_bar(load_data_from_file(sys.argv[1]),
    ...
    print(
        "Рядом с Вами находиться {}, в нем - {} мест,"
